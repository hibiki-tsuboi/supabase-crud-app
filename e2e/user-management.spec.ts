...
test.describe('User Management - E2E Smoke Tests', () => {
  ...
  test('Home page loads successfully', async ({ page }) => {
    ...
    await expect(
      page.getByRole('heading', { name: 'ユーザー管理v3' })
    ).toBeVisible()
  ...
  test('Add user page navigation and loads', async ({ page }) => {
    ...
    await expect(
      page.getByRole('heading', { name: '新しいユーザーを追加' })
    ).toBeVisible()
    ...
    await expect(page.getByText('ユーザー管理v3に戻る')).toBeVisible()
  })
  ...
  test('All page navigation flow works', async ({ page }) => {
    ...
    await page.getByRole('button', { name: '追加' }).click()
    await expect(page).toHaveURL('/users/add')
    await page.getByText('ユーザー管理v3に戻る').click()
  ...
  test('Specific user detail page loads - ddc63b72-b8c4-4888-9fd2-c0a4d1c266cd', async ({
    page,
  }) => {
    ...
    await expect(
      page.locator('h2').filter({ hasText: /^Tsuboi$/ })
    ).toBeVisible()
